...
#%% if we have the total yearly demand for each year
...
for kk in k_rng:   
    norm_hourly_demand = TimeSeriesScalerMeanVariance().fit_transform(df)
    model = TimeSeriesKMeans(n_clusters=kk, metric="dtw", dtw_inertia=True)
    model.fit(norm_hourly_demand) 
    y_pred = model.fit_predict(norm_hourly_demand)
...
# for year in range(1):
    ...
    sector_demand = np.reshape(demand[year].iloc[:,0].values, (365,24))
    
    for col in range(1,demand[year].shape[1]):
        sector_demand_other = np.reshape(demand[year].iloc[:,col].values, (365,24))
        sector_demand = np.hstack((sector_demand,sector_demand_other))
    ...
    for key in sectors_means.keys():
        ...
        reduced_demand = pd.DataFrame(np.reshape(reduced_demand.T.values, (24*(k+1), 1)))
        plt.figure()
        plt.plot(range(1,24*(k+1)+1),reduced_demand)
        plt.xlabel('Hours')
        plt.ylabel('Demand')
        plt.title(str(key)+': Reduced yearly demand, Year ' + str(year))
        
    sector_yearly_means['Year '+str(year)] = sectors_means
